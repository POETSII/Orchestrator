...
else intype = 'F';                     // Input channel is a file name
}
...
switch (intype) {
  case '0' : return EOF;               // Input channel disconnected
  case 'F' :                           // 'F': file, '*': stream
...
return c;
                                       // Non-printable kills everything
//if (isprint(c)==1) return c;           // Printable?
                                       // 'Nice' control codes?
//if (((iscntrl(c)==1)&&(isspace(c)==1))) return c;
//return EOF;                            // And finally give in
}
...
for(int i=1;i!=depth+1;i++) {
  ...
  if(i==1)dprintf(s,"      <- Token out of sequence");
  dprintf(s,"\n");
  vstr.push_back(s);
}
}
