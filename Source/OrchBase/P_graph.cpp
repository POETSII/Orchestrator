...
{
par->UnlinkAll();                      // Disconnect any/all graphs
...
pB->addr.SetBox(1);                    // Ordinal address number
pB->Build(*pConfigl.begin());          // Assemble the hardware (model)
G.InsertNode(pB->Id(),pB);             // And build the ENTIRE graph
...
void P_graph::Set2()
// Reset the topology graph, and recreate it with exactly two nodes (boxes)
{
Clear();                               // Lose any existing structure
P_box * pB = new P_box(this);          // New box
pB->Npar(this);                        // Namebase parent
...
pB->addr.SetBox(1);                    // Ordinal address number
pB->Build(*pConfigl.begin());          // Assemble the hardware (model)
G.InsertNode(pB->Id(),pB);             // Insert the first box
pB = new P_box(this);                  // Next box
pB->Npar(this);                        // Namebase parent
pB->AutoName("Bx");                    // Derive the name off the uid
pB->addr.SetBox(2);                    // Ordinal address number
pB->Build(*pConfigl.begin());          // Assemble the hardware (model)
G.InsertNode(pB->Id(),pB);             // Insert the second box
par->pPlace->Init();                   // Initialise the placement system
}
