...
{
// Send the process map in simplified form to the Motherships, and the location
// of binaries. Argument is the task name, which needs to exist and be mapped
// to hardware.  In large systems, there is a chance the user may ask to deploy
// the task before all available hardware has registered itself in the process
// map. In such a situation, a large task may not be able to be deployed
// yet. This isn't a fatal error; the user can try again and hope for success.
    if (Cl.Pa_v.size()>1) {                       // Command make sense?
        Post(47,Cl.Cl,"task","1");
        return;
    }
    ...
    map<string, P_task*>::iterator task = P_taskm.begin(); // Default to first
                                                           // task
    if (Cl.Pa_v.size()){
        string name = Cl.Pa_v[0].Val;                 // Unpack task name
        if ((task=P_taskm.find(name))==P_taskm.end()){// Is the task there?
    ...
    unsigned cIdx = 0;                        // comm number to look for
                                              // Motherships. Start from local
                                              // MPI_COMM_WORLD.
    ...
    P_thread* firstThread;  // The "first" thread in thisCore. "first" is
                            // arbitrary, because cores are stored in a map.
    ...
    WALKMAP(AddressComponent, P_box*, pE->P_boxm, boxNode)
    {
                              ...
                              unsigned, P_port*, (*board)->G, mailbox)
            {
                        ...
                        (*board)->G.NodeData(mailbox)->P_corem, core)
                {
                    ...
                    if (firstThread->P_devicel.size() &&
                        (firstThread->P_devicel.front()->par->par ==
                         task->second)) // only for cores which have something
                                        // placed on them and which belong to
                                        // the task
                    {
                        isTaskMappedToThisBox = true;
                        // determine the last thread that has a device mapped
                        // onto it (recall that all devices within a core
                        // service the same task.
                        std::map<AddressComponent, P_thread*>::reverse_iterator
                            thread;
                        ...
                        /* MLV and ADR, on 2019-01-18, discussed that the above
                         * was effectively synonymous with the below (for the
                         * new hardware model).
                        // core lists here indicate the available hardware
                        // rather than the placed hardware, so we have to
                        // search for the last placed thread.
                        unsigned t_i = 1;
                        while (t_i < thisCore->P_threadv.size() &&
                               thisCore->P_threadv[t_i]->P_devicel.size())
                               ++t_i;
                        // add it to the core list to be sent (may need to cast
                        // thisCore->addr to P_addr_t)
                        coreVec.push_back(pair<unsigned,P_addr_t>(
                            coreNum++,
                            *(dynamic_cast<P_addr_t*>(&(
                                thisCore->P_threadv[--t_i]->addr)))));
                        */
                    }
                }
            }
        }
        if ((cIdx >= Comms.size()) && coreVec.size()) // not enough Motherships
                                                      // have reported to
                                                      // deploy this task
        {
            Post(166, task->first);
        ...
        if (isTaskMappedToThisBox)
        {
            ...
            PktD.comm = PktC.comm = Comms[cIdx];  // Packet will go on the
                                                  // communicator it was found
                                                  // on
            printf("Sending a distribution message to mothership with %lu "
                   "cores\n", coreVec.size());
            PktC.Put(&coreVec); // place the core map in the packet to this
                                // Mothership
            /*
            printf("Sending a single-core distribution message to mothership "
                   "cores\n");
