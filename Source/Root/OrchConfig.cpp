...
WALKVECTOR(UIF::Node *,sects,i) {      // Walk the sections
  ...
  if (sn=="default_paths") {           // And again....
    ...
    WALKVECTOR(UIF::Node *,varis,k) {
      ...
      if ((*k)->str=="monserver_dat") default_paths.monserver_dat = s;
...
FileName::Force1Linux(default_paths.monserver_dat);
...
default_paths.monserver_dat.clear();
...
fprintf(fp,"monserver_dat = %s\n",default_paths.monserver_dat.c_str());
