...
{
  if (offset == 0) return 0; // any NULL pointer should not be reinterpreted!
...
int softswitch_onSend(ThreadCtxt_t* thr_ctxt, volatile void* send_buf)
{
    ...
    {
       tinselSetLen(hdrSize + cur_pin->pinType->sz_msg);
    ...
    while (tinselCanSend() && cur_device->currTgt < cur_pin->numTgts)
    {
	  ...
	  if (isSuperMsg)
	  {
	     set_super_hdr(tinselId() << P_THREAD_OS | ((cur_device->deviceID & P_DEVICE_MASK) << P_DEVICE_OS), cur_pin->pinType->msgType, target->tgtPin, cur_pin->pinType->sz_msg, 0, static_cast<P_Sup_Hdr_t*>(const_cast<void*>(send_buf)));
	     while (!tinselCanSend());
             tinselSend(tinselHostId(), send_buf);
	  }
          else
