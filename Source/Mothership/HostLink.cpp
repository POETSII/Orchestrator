...
void HostLink::load(const char* codeFilename, const char* dataFilename)
{
...
// Load application code and data onto a single core or thread
void HostLink::loadOne(const char* codeFilename, const char* dataFilename, uint32_t meshX, uint32_t meshY, uint32_t coreId)
{
  MemFileReader code(codeFilename);
  MemFileReader data(dataFilename);
  ...
  uint32_t addr, word;
  while (code.getWord(&addr, &word)) {
    // Send instruction to target core
...
void HostLink::startOneCore(uint32_t meshX, uint32_t meshY, uint32_t coreId, uint32_t numThreads)
{
  ...
  uint8_t flit[4 << TinselLogWordsPerFlit];
  // dump extraneous messages. Not thread-safe, so this function must be funnelled.
  ...
  // Wait for start response, dumping irrelevant responses
  // note the fragility: if the flit received contains any other
  // message type than a BootReq, the behaviour is undefined.
  while (coreStarted != dest) {
    recv(flit);
    coreStarted = ((BootReq*)flit)->args[0];
  }
}
