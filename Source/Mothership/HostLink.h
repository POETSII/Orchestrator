...
class HostLink {
 ...
 public:
  ...
  void load(const char* codeFilename, const char* dataFilename);

  // Startup (boot) a loaded application.
  void start();
  ...
  // Load application code and data onto a core
  void loadOne(const char* codeFilename, const char* dataFilename,
		   uint32_t meshX=0, uint32_t meshY=0, uint32_t coreId=0);
  
  // Startup a loaded application on one core
  void startOneCore(uint32_t meshX=0, uint32_t meshY=0, uint32_t coreId=0,
		    uint32_t numThreads=TinselThreadsPerCore);
  
  // Startup a loaded application on one thread
  inline void startOne(uint32_t meshX=0, uint32_t meshY=0, uint32_t coreId=0) {startOneCore(meshX,meshY,coreId,1);};

  // Load application and start it on one core
  inline void bootOneCore(const char* codeFilename, const char* dataFilename,
         uint32_t meshX=0, uint32_t meshY=0, uint32_t coreId=0,
			  uint32_t numThreads=TinselThreadsPerCore)
  {loadOne(codeFilename,dataFilename,meshX,meshY,coreId);
   startOneCore(meshX,meshY,coreId,numThreads);};
  
  // Load application code and data and start it on a single thread
  inline void bootOne(const char* codeFilename, const char* dataFilename,
	       uint32_t meshX=0, uint32_t meshY=0, uint32_t coreId=0)
  {loadOne(codeFilename,dataFilename,meshX,meshY,coreId);
   startOne(meshX,meshY,coreId);};
  
  // Trigger to start application execution on a single core
  void goOne(uint32_t meshX=0, uint32_t meshY=0, uint32_t coreId=0);
